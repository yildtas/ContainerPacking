...
namespace CromulentBisgetti.ContainerPacking.Algorithms
{
    public class EB_AFIT : IPackingAlgorithm
    {
        ...
        {
            Initialize(container, items);//başlat
        ...
        private void Initialize(Container container, List<Item> items)
        {
            ...
            // The original code uses 1-based indexing everywhere. 
            // Orijinal kod, her yerde 1 tabanlı indeksleme kullanır.

            // This fake entry is added to the beginning of the list to make that possible.
            // Bu sahte giriş, bunu mümkün kılmak için listenin başına eklenir.
            ...
            foreach (Item item in items)
            {
                ...
                {
                    Item newItem = new Item(item.ID, item.Length, item.Width, item.Height, item.Quantity, item.SupplierId);
