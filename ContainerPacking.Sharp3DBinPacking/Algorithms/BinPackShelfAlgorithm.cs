using ContainerPacking.Sharp3DBinPacking.Internal;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace ContainerPacking.Sharp3DBinPacking.Algorithms
{
    public class BinPackShelfAlgorithm : IBinPackAlgorithm
    {
        ...
        private void Insert(Cuboid cuboid, ShelfChoiceHeuristic method)//Sezgisel raf seçimi 
        {
            // Fazla ağırlık kontrolü
            if (cuboid.Weight + _packedCuboids.Sum(x => x.Weight) > _parameter.BinWeight)
                return;

            switch (method)
            {
                case ShelfChoiceHeuristic.ShelfNextFit://Sonraki rafa sığdır
                ...
                case ShelfChoiceHeuristic.ShelfFirstFit://İlk rafa sığdır
                    foreach (var shelf in _shelves)
                    {
                        PutOnShelf(shelf, cuboid);//Rafa koy
                        if (cuboid.IsPlaced)
                        {
                            AddToShelf(shelf, cuboid);//Rafa ekle
                            return;
                        }
                    }
                    break;
